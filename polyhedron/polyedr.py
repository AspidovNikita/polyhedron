...
class Polyedr:
    ...
    def draw(self, tk):
        # Список невидимых ребер
        tk.clean()
        sum = 0
        self.edges_uniq()
        for h in self.facets:
            z = 0
            edges_1 = []
            for i in range(0, len(h.vertexes)-1):
                edges_1.append(Edge(h.vertexes[i], h.vertexes[i+1]))
            edges_1.append(Edge(h.vertexes[len(h.vertexes)-1], h.vertexes[0]))
            for j in range(0, len(edges_1)):
                for f in self.facets:
                    edges_1[j].shadow(f)
                if len(edges_1[j].gaps) != 0:
                    z = 1
            if z == 0:
                c = h.center()
                if c.x**2 + c.y**2 + c.z**2 < 4:
                    norm = h.h_normal()
                    if math.acos(abs(norm.z) /
                                 math.sqrt(norm.x**2 +
                                           norm.y**2 +
                                           norm.z**2)) <= math.pi/7:
                        for i in range(0, len(edges_1)):
                            sum += math.sqrt((edges_1[i].beg.x -
                                              edges_1[i].fin.x)**2 +
                                             (edges_1[i].beg.y -
                                              edges_1[i].fin.y)**2)
        for e in self.edges:
